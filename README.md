...
- `src/domain/` — cœur métier, aucune dépendance à React Native, Redux ou Open Library
  ...
  - `usecases/` — `searchBooks`, `getBookDetail`, `loadLibrary`, `addBookToLibrary`, `removeBookFromLibrary`, `updateLibraryEntry`
...
- `src/store/` — state management Redux Toolkit : `librarySlice`, `searchSlice`, `bookDetailSlice` (thunks qui appellent les usecases), `store.ts`, `hooks.ts` (`useAppDispatch`/`useAppSelector`)
...
- `src/screens/` — Recherche, Fiche livre, Ma bibliothèque (branchées sur le store Redux, plus d'appel direct à l'API ou à AsyncStorage)
